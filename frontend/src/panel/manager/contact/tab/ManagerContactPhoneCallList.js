...
const ManagerContactPhoneCallList = (contactId) => {
    ...
    const updateCalls = () => {
        const fetchData = async () => {
            const result = await getCalls(contactId);
            setCallList(result);
        };
        fetchData();
    }
    ...
    const getCalls = (id) => {
        return api.post("http://localhost:8080/api/calls", {contactId:  id.contactId})
...
export default ManagerContactPhoneCallList;